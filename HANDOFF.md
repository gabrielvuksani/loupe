...
docs/adr/0001 and in Engram (topic_key architecture/loupe-loop).
...
- packages/connected also serves a long-running MCP-over-HTTP daemon (`loupe serve` =
...
Register loupe as an MCP server in your agent, select an element in the Lens, the
agent calls loupe_get_selection, edits its own repo, then calls loupe_reverify.

Verification status (be precise): the full PULL path is now proven with real components.
A gated integration test (LOUPE_LIVE_PULL) runs the built MV3 extension in headed
...
an agent uses) pulls it via loupe_get_selection. Only the LLM deciding to call the tool is
...
RESOLVED: `loupe serve` is ONE long-running daemon (WS bridge :8791 + MCP over HTTP :8792)
...
- The live agent-applies-then-reverify e2e is GATED behind LOUPE_LIVE_DISPATCH=1 so it
  ...
  fixture CSS, loupe re-rendered, the contrast finding cleared and the score rose. Codex
  was blocked by an account usage limit that day, not a loupe defect.

## Remaining for a real ship (status after the 2026-06-16 daemon session)
1. Productize: DONE. `loupe serve` is a self-contained esbuild bundle
   (packages/connected/build.mjs inlines @loupe/engine); `node dist/bin.js serve` boots
   ...
   `npx loupe serve`, and a real Windows run (only macOS verified).
...
3. Security depth: DONE. SSRF link-local/metadata blocking in assertRenderableUrl (allows
   loopback + RFC1918 dev servers). Optional one-time WS token via `loupe serve --token`
